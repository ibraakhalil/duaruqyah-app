...
const page = async () => {
    ...
    const wordsArray = [...new Set(allDuas.map((item: any) => item?.dua_name_en?.charAt(0)))].filter(item => item != undefined)
        ...
        <div className="w-full mt-4">
            {wordsArray.sort(sortingByWord).map((word: any, i: number) => <div key={i} className="mb-12">
                ...
                <div className="grid grid-cols-3 gap-x-8 gap-y-4">
                    {allDuas.filter((x: any) => word == x.dua_name_en?.charAt(0)).map((item: any, i: number) => <Link key={i} href={`/alldua/${item.dua_id}`}>
