        ...
        <div>
            <div className={cn("hidden absolute mt-5 -right-8 w-80 max-w-sm transform px-4 opacity-100 translate-y-0 z-50", { "block": show })}>
                ...
                <div className="overflow-hidden rounded-2xl shadow-lg ring-1 ring-black ring-opacity-5 ">
                    <div className="relative gap-6 bg-bg2 px-4 pt-6 pb-5">
                        {lists.map((list: any, i: number) => <Link key={i} href={`/${list.icon}`}>
                            <div className="flex items-center rounded-lg transition  hover:bg-bg cursor-pointer">
                                <div className="flex h-10 w-10 items-center justify-center">
                                    <Image className="w-5 text-bg2 stroke-black" src={`/icons/header/${list.icon}.svg`} width={24} height={24} alt={`${list.icon}`} />
                                </div>
                                <div className="">
                                    <p className="text-m ml-1 text-accent-1">{list.name}</p>
                                </div>
                            </div>
                        </Link>)}
                    </div>
